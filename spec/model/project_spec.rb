...
RSpec.describe Project do
  ...
  it "calculates github runner vcpu usage separately for metal and alien (aws) placements" do
    ...
    expect(project.current_resource_usage("GithubRunnerVCpuAlien")).to eq 0

    alien_vm = create_vm(location_id: Location::HETZNER_HEL1_ID, vcpus: 4, project_id: project.id)
    ...
    # Metal usage is unaffected by the alien runner; alien usage now reflects
    # only the AWS-placed one. This is the fix for the "sticky spillover" bug:
    # once the metal runner above finishes, GithubRunnerVCpu usage drops back
    # to 0 immediately, regardless of the still-running alien runner.
    expect(project.current_resource_usage("GithubRunnerVCpu")).to eq 2
    expect(project.current_resource_usage("GithubRunnerVCpuAlien")).to eq 4
  end
  ...
  it "calculates effective quota value" do
    ...
    expect(project.effective_quota_value("GithubRunnerVCpuAlien")).to eq 0
    expect(project.effective_quota_value("GithubRunnerVCpuArmAlien")).to eq 0
    expect(project.effective_quota_value("PostgresVCpu")).to eq 16

    project.reputation = "verified"
    expect(project.effective_quota_value("GithubRunnerVCpuAlien")).to eq 100
    expect(project.effective_quota_value("GithubRunnerVCpuArmAlien")).to eq 50
  end
