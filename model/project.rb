...
class Project < Sequel::Model
  ...
  def current_resource_usage(resource_type)
    ...
    when "GithubRunnerVCpuAlien" then GithubRunner.where(installation_id: github_installations_dataset.select(:id)).exclude(Sequel.like(Sequel[:github_runner][:label], "%-arm%")).alien_active_runner_vcpus
    when "GithubRunnerVCpuArmAlien" then GithubRunner.where(installation_id: github_installations_dataset.select(:id)).where(Sequel.like(Sequel[:github_runner][:label], "%-arm%")).alien_active_runner_vcpus
